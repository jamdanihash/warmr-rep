...
-- Recreate the function with consistent types
...
BEGIN
  ...
  WITH prospect_search AS (
    SELECT
      p.*,
      CASE
        WHEN search_term IS NULL OR search_term = '' THEN 1.0::double precision
        ELSE (
          ts_rank(
  ...
  SELECT
    ...
    ps.search_rank::double precision as relevance
  FROM prospect_search ps
  ORDER BY
    CASE
      WHEN sort_by = 'relevance' AND sort_direction = 'desc' THEN ps.search_rank END DESC NULLS LAST,
    CASE
      WHEN sort_by = 'relevance' AND sort_direction = 'asc' THEN ps.search_rank END ASC NULLS FIRST,
    CASE
      WHEN sort_by = 'company_name' AND sort_direction = 'desc' THEN ps.company_name END DESC NULLS LAST,
    CASE
      WHEN sort_by = 'company_name' AND sort_direction = 'asc' THEN ps.company_name END ASC NULLS FIRST,
    CASE
      WHEN sort_by = 'employee_count' AND sort_direction = 'desc' THEN ps.employee_count END DESC NULLS LAST,
    CASE
      WHEN sort_by = 'employee_count' AND sort_direction = 'asc' THEN ps.employee_count END ASC NULLS FIRST,
    CASE
      WHEN sort_by = 'annual_revenue' AND sort_direction = 'desc' THEN ps.annual_revenue END DESC NULLS LAST,
    CASE
      WHEN sort_by = 'annual_revenue' AND sort_direction = 'asc' THEN ps.annual_revenue END ASC NULLS FIRST;
END;
$$;